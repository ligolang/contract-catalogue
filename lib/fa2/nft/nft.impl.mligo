...
//module Operators = struct
   ...
   let is_operator (operators, owner, operator, token_id : (Datatypes.operators * address * address * nat)) : bool =
...
//module Ledger = struct
   ...
   let is_owner_of (ledger:Datatypes.ledger) (token_id : nat) (owner: address) : bool =
      let current_owner = Option.unopt (Big_map.find_opt token_id ledger) in
      current_owner = owner

   let assert_owner_of (ledger:Datatypes.ledger) (token_id : nat) (owner: address) : unit =
      assert_with_error (is_owner_of ledger token_id owner) Errors.ins_balance

   let transfer_token_from_user_to_user (ledger : Datatypes.ledger) (token_id : nat) (from_ : address) (to_ : address) :Datatypes.ledger =
...
//module Storage = struct
   ...
   let is_owner_of (s:Datatypes.storage) (owner : address) (token_id : nat) : bool =
   ...
   let get_balance (s : Datatypes.storage) (owner : address) (token_id : nat) : nat =
...
[@entry] let transfer (t:TZIP12.transfer) (s:storage) : operation list * storage =
   ...
   let process_atomic_transfer (from_:address) (ledger, t:ledger * TZIP12.atomic_trans) =
      ...
      let ledger = Sidecar.transfer_token_from_user_to_user ledger token_id from_ to_ in
      ledger
   in
...
[@entry] let balance_of (b: TZIP12.balance_of) (s: Datatypes.storage) : operation list * storage =
   let {requests;callback} = b in
   let get_balance_info (request : TZIP12.request) : TZIP12.callback =
      let {owner;token_id} = request in
      let balance_ = Sidecar.get_balance s owner token_id in
...
[@view] let get_balance (p : address * nat) (s : Datatypes.storage) =
      let (owner, token_id) = p in
      let balance_ = Sidecar.get_balance s owner token_id in
      balance_

[@view] let total_supply (token_id :nat) (s : Datatypes.storage) =
      let () = Assertions.assert_token_exist s.token_metadata token_id in
      1n

[@view] let all_tokens (() : unit) (_s : Datatypes.storage) : nat set = failwith Errors.not_available

[@view] let is_operator (op : TZIP12.operator) (s : Datatypes.storage) : bool =
 let authorized = match Big_map.find_opt (op.owner, op.operator) s.operators with
            Some(opSet) -> opSet
            | None -> Set.empty in
    Set.size authorized > 0n || op.owner = op.operator

[@view] let token_metadata (p : nat) (s : Datatypes.storage) : TZIP12.tokenMetadataData =
